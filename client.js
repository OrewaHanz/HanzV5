      ...
      if (connection === 'connecting') {
       spinnies.add('start', {
         ...
         spinnies.succeed('start', {
            text: `Connected, you login as PutXd (Owner)`
         })
   ...
   client.ev.on('group-participants.update', async (room) => {
      ...
      if (room.action == 'add') {
         if (groupSet && groupSet.localonly) {
            if (global.db.users.some(v => v.jid == member) && !global.db.users.find(v => v.jid == member).whitelist && !member.startsWith('60') || !member.startsWith('60')) {
               client.reply(room.id, Func.texted('bold', `Sorry @${member.split`@`[0]}, this group is only for malaysian people and you will removed automatically.`))
